                                ...
                                <div className='pr-3 md-lg:pr-0'>
                                    <div className='flex flex-col gap-3'>
                                        <div className='bg-white p-4'>
                                            <h2 className='text-md text-green-500 font-semibold'>Stock Productos {card_products.length}</h2>
                                        </div>
                                        {
                                                    ...
                                                    p.products.map((pt, i) => <div className='w-full flex flex-wrap'>
                                                        ...
                                                        <div className='flex justify-between w-5/12 sm:w-full sm:mt-3'>
                                                            <div className='pl-4 sm:pl-0'>
                                                                <h2 className='text-lg text-orange-500'>Bs{pt.productInfo.price - Math.floor((pt.productInfo.price * pt.productInfo.discount) / 100)} </h2>
                                        ...
                                        }
                                        {
                                            outofstock_products.length > 0 && <div className='flex flex-col gap-3'>
                                                <div className='bg-white p-4'>
                                                    <h2 className='text-md text-red-500 font-semibold'>Agotado  {outofstock_products.length}</h2>
                                                </div>
                                                <div className='bg-white p-4'>
                                                    {
                                ...
                                <div className='pl-3 md-lg:pl-0 md-lg:mt-5'>
                                    {
                                        card_products.length > 0 && <div className='bg-white p-3 text-slate-600 flex flex-col gap-3'>
                                            ...
                                            <div className='flex justify-between items-center'>
                                                <span>{buy_product_item} Artículo</span>
                                                <span>Bs{price}</span>
                                            </div>
                                            <div className='flex justify-between items-center'>
                                                <span>Tarifa de envío</span>
                                                <span>Bs{shipping_fee}</span>
                                            </div>
                                            <div className='flex gap-2'>
                                                <input className='w-full px-3 py-2 border border-slate-200 outline-0 focus:border-green-500 rounded-sm' type="text" placeholder='Cupon de descuento' />
                                                <button className='px-5 py-[1px] bg-blue-500 text-white rounded-sm uppercase text-sm'>Aplicar</button>
                                            </div>
                                            <div className='flex justify-between items-center'>
                                                <span>Total</span>
                                                <span className='text-lg text-orange-500'>Bs{price + shipping_fee}</span>
                                            </div>
                                            <button onClick={redirect} className='px-5 py-[6px] rounded-sm hover:shadow-orange-500/20 hover:shadow-lg bg-orange-500 text-sm text-white uppercase'>Proceder al Pago {buy_product_item}</button>
                                        </div>
                                    }
                                </div>
                            </div>
